...
/*     @ApiBearerAuth('bearer') // Связка со Swagger
    ...
    async verifyUserForDoor( @Body() dto: VerifyUserDto) {
    ...
    async getAllBuildingDoors() {
        return await this.doorService.getAlldoors();
    }
    ...
    @Roles(['building-admin', 'system-admin'])
    @UseGuards(RoleGuard)
    @Get('/statistics/access-anomalies')
    @ApiOperation({ summary: 'Find access anomalies (users with excessive access)' })
    async getAccessAnomalies() {
        try {
            return await this.doorService.getAccessAnomalies();
        } catch (error) {
            throw new HttpException(
                error.message,
                error.status || HttpStatus.INTERNAL_SERVER_ERROR,
